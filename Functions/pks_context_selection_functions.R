...
select_context_snv = function(gr) {
  ...
  chr = seqnames(gr)
  start = start(gr) - 3 %>% as.integer()
  end = end(gr) + 3 %>% as.integer()
  strand = strand(gr) %>% as.character()
  contexts = getSeq(Hsapiens, names = chr, start =start , end = end ,strand = strand, as.character = T )

  gr_snv_pks_T = gr[gr$REF == "T" & substring(contexts, 1,1) == "A" & grepl("[TA][TA][TA]", substring(contexts, 3,5))] 
  gr_snv_pks_A = gr[gr$REF == "A" & substring(contexts, 1,1) == "A" & grepl("[TA][TA][TA]", substring(contexts, 3,5))]
  gr_snv_pks = unlist(GRangesList(gr_snv_pks_A, gr_snv_pks_T))
  return(gr_snv_pks)
}
...
select_context_indel = function(gr, type = 'cBioPortal'){
  ...
  if (type == "cBioPortal") {
    ...
    start = start(gr_dels ) - 8 %>% as.integer()
    end = end(gr_dels ) + 8 %>% as.integer()
    ...
    if (length(gr_dels) == 0 ) {return()}
    chr = seqnames(gr_dels)
    start = end(gr_dels ) - 8 %>% as.integer()
    end = end(gr_dels ) + 8 %>% as.integer()
    strand = strand(gr_dels ) %>% as.character()
    contexts = getSeq(Hsapiens, names = chr, start =start , end = end ,strand = strand, as.character = T )
